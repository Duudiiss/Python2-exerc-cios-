...
#     o descrito e construa e retorne um dicionário com
#     CHAVE: ANO
...
# A) =========================================================
def vencedorNoAno(dicCampeonato, modalidade, ano):
    dicModalidade = dicCampeonato.get(modalidade, True)
    if dicModalidade == True:
        print('%s não é modalidade no campeonato' %modalidade)
    else:
        vencedor = dicModalidade['ano'].get(ano, True)
        if vencedor == True:
            print('Ano %d - ano inexistente'%ano)
        else:
            
            print('Mod: %s - Ano %d - %s'%(modalidade, ano, vencedor))
# B) =========================================================   
def incluiCompetidor(dicCampeonato, modalidade, nome, peso):
    dicModalidade = dicCampeonato.get(modalidade, True)
    if dicModalidade == True:
        print('%s não é modalidade no campeonato' %modalidade)
    else:
        minimo = dicModalidade['pesos'].get('minimo')
        maximo = dicModalidade['pesos'].get('maximo')
        if peso > minimo and peso < maximo:
            dicCampeonato[modalidade]['inscritos'][nome] = peso
# C) =========================================================
def criaDicPorAno(dicCampeonato):
    dicAnoGanhador = dict()
    for categoria in dicCampeonato:
        dicAno = dicCampeonato[categoria]['ano']
        for ano in dicAno:
            if ano in dicAnoGanhador:
                dicAnoGanhador[ano][categoria] = dicAno[ano]
            else:
                dicAnoGanhador[ano] = {categoria:dicAno[ano]}
            
            
    return dicAnoGanhador
